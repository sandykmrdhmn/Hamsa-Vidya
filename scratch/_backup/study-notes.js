...
class StudyNotesView {
  ...
  renderDashboardView() {
    ...
    this.container.innerHTML = `
      <div class="study-vault-container">
        <!-- Hero Section Header -->
        <div class="study-vault-hero">
          <div>
            <div class="study-vault-badge">
              <i data-lucide="book-marked" style="width:16px;height:16px;"></i>
              <span>AI Digital Textbook & Study Workspace</span>
            </div>
            <h1 class="study-vault-title gradient-text">
              Study Notes & Digital Textbooks
            </h1>
            <p class="study-vault-subtitle">
              Import textbook chapters (PDF) or photos of notes (JPG/PNG). Gemini AI transforms them into color-coded digital textbooks with smart glossaries, interactive examples, and instant quizzes.
            </p>
          </div>

          <div class="study-hero-actions">
            <button class="btn btn-primary btn-hero-import" onclick="studyNotesView.openCreateNoteModal()">
        ...
        <!-- Metric Statistics Ribbon -->
        <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap:1rem; margin-bottom:2rem;">
          <div class="metric-card" style="padding:1.1rem 1.3rem;">
            <div class="metric-header">
              <span>Total Textbooks</span>
              <i data-lucide="book-open" style="width:18px;height:18px;color:#6366f1;"></i>
            </div>
            <div class="metric-value" style="font-size:1.8rem;">${this.notes.length}</div>
            <div class="metric-caption">Saved study modules</div>
          </div>
          ...
          <div class="metric-card" style="padding:1.1rem 1.3rem;">
            ...
            <div class="metric-value" style="font-size:1.8rem; color:#f59e0b;">${starredCount}</div>
            <div class="metric-caption">Starred for quick revision</div>
          </div>
          ...
          <div class="metric-card" style="padding:1.1rem 1.3rem;">
            ...
            <div class="metric-value" style="font-size:1.8rem; color:#10b981;">${totalWordsAll.toLocaleString()}</div>
            <div class="metric-caption">Words ingested & structured</div>
          </div>

          <div class="metric-card" style="padding:1.1rem 1.3rem;">
            <div class="metric-header">
              <span>Study Time</span>
              <i data-lucide="clock" style="width:18px;height:18px;color:#ec4899;"></i>
            </div>
            <div class="metric-value" style="font-size:1.8rem; color:#ec4899;">~${totalReadingHours}h</div>
            <div class="metric-caption">Cumulative reading coverage</div>
          </div>
        </div>
  ...
  renderCreateView() {
    this.container.innerHTML = `
      <div class="study-vault-container" style="max-width:100%; width:100%;">
  ...
  downloadSourceText(noteId) {
    ...
    a.href = url;
    a.download = `${(note.title || 'study-note').replace(/[^a-zA-Z0-9_-]/g, '_')}_original_source.txt`;
    a.click();
  ...
  highlightSelectedText(color) {
    ...
    try {
      ...
      console.log('Highlight visual fallback', e);
    }
