...
class ToolsView {
  ...
  renderToolsHub() {
    ...
    this.container.innerHTML = `
      <div class="tools-studio-wrapper full-width-page">
        <!-- Hero Header -->
        <header class="tools-hero-header">
          <div class="tools-hero-inner">
            <div class="tools-hero-badge">
              <span class="pulse-dot"></span>
              <span>100% Client-Side Private • Zero Server Upload • Instant Processing</span>
            </div>
            <div class="tools-title-row">
              <h1 class="tools-hero-title">
                <i data-lucide="wrench" style="width:34px;height:34px;color:var(--color-primary-light);"></i>
                <span>Hamsa Study & Document Utilities Suite</span>
              </h1>
            </div>
            <p class="tools-hero-subtitle">
              Professional, fully functional image, PDF, writing, and focus utilities built specifically for UPSC, SSC, state PSCs, and competitive exam aspirants.
            </p>
  ...
  initImageCompressorEvents() {
    ...
    if (downloadBtn) {
      downloadBtn.onclick = () => {
        ...
        link.href = URL.createObjectURL(this.imgCompressState.resultBlob);
        link.download = `${rawName}-compressed.${ext}`;
        link.click();
  ...
  initImageResizerEvents() {
    ...
    if (downloadBtn) {
      downloadBtn.onclick = () => {
        ...
        const link = document.createElement('a');
        link.href = URL.createObjectURL(this.imgResizeState.resultBlob);
        link.download = `${rawName}-${this.imgResizeState.targetW}x${this.imgResizeState.targetH}.${ext}`;
        link.click();
        if (window.audioEngine) window.audioEngine.playCelebration();
  ...
  async executePdfSplit() {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `${rawName}-extracted-${rangeTag}.pdf`;
      link.click();
  ...
  async executePdfCompression() {
    ...
    try {
      ...
      link.download = `${rawName}-compressed.pdf`;
      link.click();
  ...
  async executeImagesToPdf() {
    ...
    try {
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `hamsa-compiled-notes.pdf`;
      link.click();
